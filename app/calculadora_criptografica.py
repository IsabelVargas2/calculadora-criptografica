# -*- coding: utf-8 -*-
"""
Calculadora Criptografica - Taller de Ciberseguridad
Autora: Maria Isabel Vargas Mendoza (Isa)
UNAB - Ingenieria de Sistemas

Archivo unico: contiene tanto la logica criptografica/matematica como la
interfaz Streamlit, para que el taller se entregue como un solo script .py
...
if submenu.startswith("1.1"):
    ...
    if st.button("Calcular"):
        r = inverso_multiplicativo_tradicional(int(a), int(n))
    ...
    n = st.number_input("n (módulo)", value=26, step=1, min_value=2)
    if st.button("Calcular"):
        r = inverso_multiplicativo_aee(int(a), int(n))
...
# ------------------------------------------------------------------
    ...
    if st.button("Calcular"):
        r = cifrado_modulo27(texto, int(k), modo)
    ...
    modo = st.radio("Modo", ["cifrar", "descifrar"], horizontal=True)
    if st.button("Calcular"):
        r = cifrado_cesar(texto, int(k), modo)
        mostrar(r, {"Resultado": r["resultado"]})
    ...
    if st.button("Calcular"):
        r = cifrado_vernam(texto, clave, modo)
        mostrar(r, {"Resultado": r["resultado"]})

elif submenu.startswith("2.4"):
    texto = st.text_input("Texto", value="HOLA MUNDO")
    if st.button("Calcular"):
        r = cifrado_atbash(texto)
        mostrar(r, {"Resultado": r["resultado"]})
    ...
    modo = st.radio("Modo", ["cifrar", "descifrar"], horizontal=True)
    if st.button("Calcular"):
        r = cifrado_transposicion_columnar(texto, clave.upper(), modo)
    ...
    if st.button("Calcular"):
        try:
            r = cifrado_afin(texto, int(a), int(b), modo)
    ...
    modo = st.radio("Modo", ["cifrar", "descifrar"], horizontal=True)
    if st.button("Calcular"):
        r = cifrado_sustitucion_simple(texto, clave_sust or None, modo)
        mostrar(r, {"Resultado": r["resultado"]})
...
# ------------------------------------------------------------------
    ...
    if st.button("Calcular"):
        r = diffie_hellman(int(p), int(g), int(a_priv), int(b_priv))
    ...
    if st.button("Calcular"):
        r = exponenciacion_rapida(int(base), int(exp), int(mod))
        mostrar(r, {"Resultado": r["resultado"]})
...
# ------------------------------------------------------------------
    ...
    if st.button("Calcular"):
        r = codificar_binario(texto, modo)
        mostrar(r, {"Resultado": r["resultado"]})
...
# ------------------------------------------------------------------
    ...
    if st.button("Calcular"):
        r = hash_con_salts(clave, "md5", [s.strip() for s in salts_txt.split(",")])
        mostrar(r)
    ...
    salts_txt = st.text_input("Salts (separados por coma)", value="a1B9,z7Kq,Xy02,salt_2026")
    if st.button("Calcular"):
        r = hash_con_salts(clave, "sha256", [s.strip() for s in salts_txt.split(",")])
        mostrar(r)
    ...
    salts_txt = st.text_input("Salts (separados por coma)", value="a1B9,z7Kq,Xy02,salt_2026")
    if st.button("Calcular"):
        r = hash_con_salts(clave, "sha512", [s.strip() for s in salts_txt.split(",")])
        mostrar(r)
